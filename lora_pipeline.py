...
from diffusers.utils.peft_utils import _create_lora_config
...
def _load_lora_into_text_encoder(
	...
	if len(state_dict) > 0:
		text_keys={
			...
			"to_q_lora":"q_proj",
			"to_v_lora":"v_proj",
			"lora_linear_layer.down.weight":"lora_A.weight",
			"lora_linear_layer.up.weight":"lora_B.weight",
			".down.weight":".lora_A.weight",
			".up.weight":".lora_B.weight",
		}
		...
		for k in all_lora_keys:
			...
			for k2 in text_keys:
				if k2 in mk:
					mk=mk.replace(k2,text_keys[k2])
			if k!=mk:
				state_dict[mk]=state_dict[k]
				del state_dict[k]

		for name, _ in text_encoder.named_modules():
			if name.endswith((".q_proj", ".k_proj", ".v_proj", ".o_proj", ".fc1", ".fc2")):
...
def _split_text(sd,text_encoder_name):
	...
	for k in all_lora_keys:
		if k.startswith("lora_te_"):
			...
			diffusers_name = _convert_text_encoder_lora_key(k, lora_name)
			text_sd[diffusers_name] = sd.pop(k)
			text_sd[diffusers_name.replace(".down.weight", ".up.weight")] = sd.pop(lora_name_up)
	...
	text_sd = {f"{text_encoder_name}.{module_name}": params for module_name, params in text_sd.items()}
	return trans_sd,text_sd,network_alphas
...
class AnimaLoraLoaderMixin(LoraBaseMixin):
	...
	def lora_state_dict(
		...
		**kwargs,
		...
		state_dict,text_sd,network_alphas=_split_text(state_dict,cls.text_encoder_name)

		converted_state_dict = _convert_non_diffusers_anima_lora_to_diffusers(
			state_dict
		)
		if return_lora_metadata:
			return converted_state_dict, metadata,text_sd,network_alphas
		return converted_state_dict,text_sd,network_alphas
	...
	def load_lora_weights(
		...
		**kwargs,
		...
		state_dict, metadata,text_sd,network_alphas = self.lora_state_dict(
	...
	def save_lora_weights(
		...
		if not lora_layers:
			raise ValueError("`transformer_lora_layers` or `text_encoder_lora_layers` must be provided.")
