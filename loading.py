        ...
        for k in keys:
            if k.startswith(head):
                ...
                if not(mk in pass_keys):
                    current[mk]=current[k]
            del current[k]
    return current


def _vae_text_check(model_path):
    keys1={
        "conv1":"quant_conv",
...
def build_anima_pipeline(
    ...
    *,
    ...
    vae_sd,text_sd=_vae_text_check(resolved_model_path)
    if vae_sd=={}:
        vae = load_vae(
            device=load_device,
            dtype=resolved_dtype,
        ...
        vae=AutoencoderKLQwenImage.from_config(ANIMA_VAE_CONFIG)
        for k,p in vae.named_parameters():
            p.data=vae_sd[k].to(resolved_dtype)
        vae.to(load_device)
    ...
    if text_sd=={}:
        ...
        text_encoder=Qwen3Model(Qwen3Config(**QWEN3_06B_CONFIG))
        for k,p in text_encoder.named_parameters():
            p.data=text_sd[k].to(resolved_dtype)
